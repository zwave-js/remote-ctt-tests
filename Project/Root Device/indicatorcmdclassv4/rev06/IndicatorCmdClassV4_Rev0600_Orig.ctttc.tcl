PACKAGE IndicatorCmdClassV4_Rev0600_Orig; // do not modify this line
...
TESTSEQ IndividualValuesNodeIdentify: "Set individual values for properties 0x03, 0x04, 0x05 of Indicator ID 0x50 (Node Identify)"
    ...
    LOOP ($i; 0; LENGTH($property03Values) - 1) // LOOP for each configured Indicator ID
    {
        ...
        LOOP ($obj; 0; $objCount - 1)
        {
            ...
            {
                $prop5IsPresent = 1;
                $prop5ObjValue = $objects[3 * $obj + 2];

                IF ($property05Values[$i] == 0x00)
                {
                    $expValue = $property03Values[$i] / 2;
                    $isUneven = 0;
                    ...
                    IF ( $prop5ObjValue == 0x00 && $prop5ObjValue != $expValue && ( $isUneven == 0 || $prop5ObjValue != ($expValue + 1) ) )
                    {
                        MSG ("Warning: The set value 0x00 MUST represent symmetric On/Off period (On time equal to Off time). In this case, the actual value (half the On/Off period) SHOULD be reported.");

                        IF ($isUneven == 1)
                        {
                            MSG ("Warning: Expected was: 0x{0:X2} or 0x{1:X2}", UINT($expValue), UINT($expValue + 1));
                        }
                        ELSE
                        {
                            MSG ("Warning: Expected was: 0x{0:X2}", UINT($expValue));
                        }
                    }
                    ELSEIF ( $prop5ObjValue != $expValue || ( $isUneven == 1 && $prop5ObjValue != ($expValue + 1) ) )
                    {
                        IF ($isUneven == 1)
                        {
                            MSGFAIL ("Mismatch in Property 0x05 values detected. Expected was: 0x{0:X2} or 0x{1:X2}", UINT($expValue), UINT($expValue + 1));
                        }
                        ELSE
                        {
                            MSGFAIL ("Mismatch in Property 0x05 values detected. Expected was: 0x{0:X2}", UINT($expValue));
                        }

                        MSGFAIL ("The set value 0x00 MUST represent symmetric On/Off period (On time equal to Off time). In this case, the actual value (half the On/Off period) SHOULD be reported. Alternatively, 0x00 MAY be reported.");
                    }
                }
                ELSEIF ($prop5ObjValue != $property05Values[$i])
                {
                    MSGFAIL ("Mismatch in Property 0x05 values detected. Expected was: 0x{0:X2}", UINT($property05Values[$i]));
                }
            }
        }
