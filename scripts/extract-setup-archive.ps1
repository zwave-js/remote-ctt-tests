...
.DESCRIPTION
    This script extracts setup.zip and places files in the correct locations:
...
$archiveFile = Join-Path $repoRoot "setup.zip"
...
# Copy zwave-js-cache -> .zwave-js-cache
$sourceCache = Join-Path $tempDir "zwave-js-cache"
if (Test-Path $sourceCache) {
    Write-Host "Copying zwave-js-cache -> .zwave-js-cache/" -ForegroundColor Green
    if (Test-Path $zwaveJsCache) {
        Remove-Item -Recurse -Force $zwaveJsCache
    }
    Copy-Item -Recurse $sourceCache $zwaveJsCache
} else {
    Write-Host "WARNING: zwave-js-cache/ not found in archive" -ForegroundColor Yellow
}
...
Write-Host "  - $zwaveJsCache" -ForegroundColor White
