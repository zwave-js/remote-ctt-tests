...
const ZWAVE_DEVICES: ZWaveDevice[] = [
  {
    name: "Controller 1 (Z-Wave JS)",
    port: 5000,
    binary: "controller",
    storageDir: "/tmp/controller1",
  },
  {
    name: "Controller 2 (CTT)",
    port: 5001,
    binary: "controller",
    storageDir: "/tmp/controller2",
  },
  {
    name: "Controller 3 (CTT)",
    port: 5002,
    binary: "controller",
    storageDir: "/tmp/controller3",
  },
  {
    name: "End Device 1 (CTT)",
    port: 5003,
    binary: "end_device",
    storageDir: "/tmp/enddevice1",
  },
  {
    name: "End Device 2 (CTT)",
    port: 5004,
    binary: "end_device",
    storageDir: "/tmp/enddevice2",
  },
];
...
class ProcessManager {
    ...
    return new Promise((resolve, reject) => {
      dockerProcess.on("close", (code) => {
        if (code === 0) {
          ...
          reject(
            new Error(`Docker container failed to start with code ${code}`)
          );
        }
      });
    });
  ...
  startZWaveStackNative(): void {
    ...
    const controllerBinary =
      "ZW_zwave_ncp_serial_api_controller_25_9_0_x86_REALTIME_DEBUG.elf";
    const endDeviceBinary =
      "ZW_zwave_ncp_serial_api_end_device_25_9_0_x86_REALTIME_DEBUG.elf";

    for (const device of ZWAVE_DEVICES) {
      const binaryName =
        device.binary === "controller" ? controllerBinary : endDeviceBinary;
      ...
      const proc = spawn(
        binaryPath,
        ["--port", device.port.toString(), "--storage", device.storageDir],
        {
          cwd: zwaveStackDir,
          stdio: "pipe",
        }
  ...
  async startZWaveJSServer(): Promise<void> {
    ...
    this.zwaveDriver = new Driver("tcp://127.0.0.1:5000", {
      ...
      securityKeys: {
        S2_Unauthenticated: Buffer.from("CE07372267DCB354DB216761B6E9C378", "hex"),
        S2_Authenticated: Buffer.from("30B5CCF3F482A92E2F63A5C5E218149A", "hex"),
        S2_AccessControl: Buffer.from("21A29A69145E38C1601DFF55E2658521", "hex"),
        S0_Legacy: Buffer.from("C6D90542DE4E66BBE66FBFCB84E9FF67", "hex"),
      },
      securityKeysLongRange: {
        S2_Authenticated: Buffer.from("0F4F7E178A4207A0BBEFBF991C66F814", "hex"),
        S2_AccessControl: Buffer.from("72D42391F7ECE63BE1B38B25D085ECD4", "hex"),
      },
    });
  ...
  setupWineAppData(): void {
    ...
    const wineAppDataRoaming = path.join(
      winePrefix,
      "drive_c",
      "users",
    ...
    const programFilesX86 = path.join(
      winePrefix,
      "drive_c",
      "Program Files (x86)"
    );
    ...
    const literalProgramFilesX86 = path.join(
      cttRemoteDir,
      "%ProgramFiles(x86)%"
    );
  ...
  startWebSocketServer(): void {
    this.wsServer = createWebSocketServer({
      ...
      onProjectLoaded: () => {
        ...
        setTimeout(async () => {
          try {
            console.log("\n--- Running test case DD_VersionCCData_Rev01 ---");
            const result = await runTestCases({
              testCaseNames: ["DD_VersionCCData_Rev01"],
              endPointIds: [0],
